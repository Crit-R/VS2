...
public:
	...
	VanitySearch(Secp256K1 *secp, std::string minn, std::string maxx, std::vector<std::string> &prefix, std::string seed, int nbit, int searchMode,
		int coinType, bool useGpu, bool stop, std::string outputFile, bool useSSE, uint32_t maxFound, uint64_t rekey,
...
private:
	...
	bool checkPrivKeyETH(std::string addr, Int &key, int32_t incr, int endomorphism);
	...
	void checkAddrETH(int prefIdx, uint8_t *hashETH, Int &key, int32_t incr, int endomorphism);
	...
	void checkAddressesETH(Int key, int i, Point p1);
	void output(std::string addr, std::string pAddr, std::string pAddrHex, int pvkBits);
	void outputETH(std::string addr, std::string pAddrHex, int pvkBits);
