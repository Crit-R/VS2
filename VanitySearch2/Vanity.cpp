...
VanitySearch::VanitySearch(Secp256K1 *secp, string minn, string maxx, vector<std::string> &inputPrefixes, string seed,
	int nbit, int searchMode, int coinType,
	...
	if (rekey > 0) {
		printf("Base Keys: Randomly changed every %.0f Mkeys\n", (double)rekey);
	}
	//else {
	printf("Base Key bit: %d\n", nbit);
	//}
	char* cstr = &minn[0];
	m1.SetBase16(cstr);
	char* cstr2 = &maxx[0];
...
uint64_t getBitLength(uint64_t x)
{
	return 64ULL - LZC2(x);
}
...
bool VanitySearch::initPrefix(std::string &prefix, PREFIX_ITEM *it) {
	...
	if (coinType == COIN_BTC) {
		...
		auto address = prefix;
		address.erase(0, 2);
		int prefixSize = address.size();
		if (prefixSize % 2 != 0) {
			printf("Wrong eth address prefix, must be in even number of characters\n");
			return false;
		}
		//for (int i = 0; i < prefixSize; i += 2) {
		//	uint8_t c = 0;
		...
		result.reserve((address.size() + width - 1) / width);
		for (auto ita = address.crbegin(); ita < address.crend(); ita += width) {
			auto begin = MIN(address.crend(), ita + width).base();
			auto end = ita.base();
		...
		it->sPrefix = *(prefix_t *)(result.data() + 0);

		// Difficulty
		//it->difficulty = pow(2, 160); // pow(2, 192) / pow(58, nbDigit);
		it->isFull = false;
		it->lPrefix = 0;
		it->prefix = (char *)prefix.c_str();
		it->prefixLength = (int)prefix.length();
		memcpy(it->hash160, result.data(), result.size());


		//uint8_t hashIntBytes[32];
		//uint8_t hashIntBytes2[32];
		//memset(hashIntBytes, 0, 32);
		//memcpy(hashIntBytes, result.data(), result.size());

		//uint64_t *hashIntBytes64 = (uint64_t *)hashIntBytes;
		//uint64_t *hashIntBytes264 = (uint64_t *)hashIntBytes2;

		//hashIntBytes264[3] = hashIntBytes64[0];
		//hashIntBytes264[2] = hashIntBytes64[1];
		//hashIntBytes264[1] = hashIntBytes64[2];
		//hashIntBytes264[0] = hashIntBytes64[3];

		int bitLength = getBitLength(result.at(0)) + (result.size() - 1) * 8;

		//Int hashInt;
		//hashInt.Set32Bytes(hashIntBytes2);

		it->difficulty = pow(2, bitLength);

		//std::cout << "bitLength: " << bitLength << std::endl;

		// Difficulty
		//it->difficulty = pow(256, prefix.length() - 1); // pow(2, 5 * (prefix.length() - 2));

		//std::cout << std::endl << std::endl << std::endl;
		//std::cout << "initPrefix: " << std::endl;
		//for (int i = 0; i < 20; i++) {
		//	printf("%02x", ((uint8_t*)it->hash160)[i]);
		//}
		//printf("\n");
		//std::cout << "sPrefix: " << it->sPrefix << std::endl;
		//std::cout << it->prefix << std::endl << std::endl << std::endl;
...
void VanitySearch::output(string addr, string pAddr, string pAddrHex, int pvkBits) {
	...
	if (startPubKeySpecified) {
		...
		fprintf(f, "Priv (HEX): 0x%s (%d bit)\n", pAddrHex.c_str(), pvkBits);
...
void VanitySearch::outputETH(std::string addr, std::string pAddrHex, int pvkBits) {
...
bool VanitySearch::checkPrivKey(string addr, Int &key, int32_t incr, int endomorphism, bool mode) {
	...
	output(addr, secp->GetPrivAddress(mode, k), k.GetBase16(), k.GetBitLength());
...
bool VanitySearch::checkPrivKeyETH(string addr, Int &key, int32_t incr, int endomorphism) {
	...
	string chkAddr = secp->GetAddressETH(p);
	if (chkAddr != addr) {
		if (nbit == 256) {
			//Key may be the opposite one (negative zero or compressed key)
			k.Neg();
	...
	outputETH(addr, k.GetBase16(), k.GetBitLength());
...
// ----------------------------------------------------------------------------
void VanitySearch::checkAddrETH(int prefIdx, uint8_t *hashETH, Int &key, int32_t incr, int endomorphism)
{

	if (hasPattern) {

		// Wildcard search
		string addr = secp->GetAddressETH(hashETH);
		...
		for (int i = 0; i < (int)inputPrefixes.size(); i++) {
			...
			if (Wildcard::match(addr.c_str(), inputPrefixes[i].c_str(), caseSensitive)) {
				...
				// Found it !
				//*((*pi)[i].found) = true;
				if (checkPrivKeyETH(addr, key, incr, endomorphism)) {
					nbFoundKey++;
					patternFound[i] = true;
					updateFound();
				}
	...
	if (onlyFull) {
		...
		for (int i = 0; i < (int)pi->size(); i++) {
			...
			if (keccak160_comp_hash((*pi)[i].hash160, hashETH)) {

				// Found it !
				*((*pi)[i].found) = true;
				// You believe it ?
				if (checkPrivKeyETH(secp->GetAddressETH(hashETH), key, incr, endomorphism)) {
					nbFoundKey++;
					updateFound();
				}
		...
		for (int i = 0; i < (int)pi->size(); i++) {
			...
			//std::cout << "addr: " << (*pi)[i].prefix << " : " << a << std::endl;
...
// ----------------------------------------------------------------------------
void VanitySearch::checkAddressesETH(Int key, int i, Point p1)
{
	unsigned char h0[20];
	...
	if (nbit == 256) {
		...
		secp->GetHashETH(pte1[0], h0);

		pr0 = *(prefix_t *)h0;
		if (hasPattern || prefixes[pr0].items)
			checkAddrETH(pr0, h0, key, i, 1);

		// Endomorphism #2
		pte2[0].x.ModMulK1(&p1.x, &beta2);
		pte2[0].y.Set(&p1.y);

		secp->GetHashETH(pte2[0], h0);

		pr0 = *(prefix_t *)h0;
		if (hasPattern || prefixes[pr0].items)
			checkAddrETH(pr0, h0, key, i, 2);
...
void VanitySearch::FindKeyCPU(TH_PARAM *ph) {
	...
	while (!endOfSearch) {
		...
		if (coinType == COIN_BTC) {
			...
			for (int i = 0; i < CPU_GRP_SIZE && !endOfSearch; i++) {
				checkAddressesETH(key, i, pts[i]);
			}
		}
		key.Add((uint64_t)CPU_GRP_SIZE);
		counters[thId] += (nbit == 256 ? 6ULL : 1ULL) * CPU_GRP_SIZE; // Point + endo #1 + endo #2 + Symetric point + endo #1 + endo #2
	...
	while (ok && !endOfSearch) {
		...
		for (int i = 0; i < (int)found.size() && !endOfSearch; i++) {
			...
			if (coinType == COIN_BTC) {
				...
				checkAddrETH(*(prefix_t *)(it.hash), it.hash, keys[it.thId], it.incr, it.endo);
			}
		...
		if (ok) {
			...
			counters[thId] += (nbit == 256 ? 6ULL : 1ULL) * STEP_SIZE * nbThread;
		}
