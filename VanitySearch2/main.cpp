...
void printUsage() {
	...
	printf("VanitySeacrh [-check] [-v] [-u] [-b] [-c] [-gpu] [-stop] [-coin cointype] [-i inputfile]\n");
	printf("             [-gpuId gpuId1[,gpuId2,...]] [-g g1x,g1y,[,g2x,g2y,...]]\n");
	printf("             [-o outputfile] [-m maxFound] [-ps seed] [-s seed] [-t nbThread]\n");
	printf("             [-bit bitNumber] [-nosse] [-r rekey] [-check] [-kp] [-sp startPubKey]\n");
	...
	printf(" -coin: coint type, btc or eth\n");
	...
	printf(" -bit bitNumber: Specify bit length for private key, default is 256 bit\n");
	...
	printf(" -min minimum private key hex for random \n");
	printf(" -max maximum private key hex for random \n");
	exit(0);
...
int parseCoinType(const std::string& s)
{
	...
	printf("Invalid %s coin argument\n", s.c_str());
	exit(-1);
}
// ------------------------------------------------------------------------------------------
...
int main(int argc, char* argv[]) {
	...
	while (a < argc) {
		...
		if (strcmp(argv[a], "-gpu") == 0) {
			...
			a++;
			std::string coin = string(argv[a]);
			coinType = parseCoinType(coin);
			a++;
		}
		...
		else if (strcmp(argv[a], "-bit") == 0) {
			a++;
			nbit = getInt("bit", argv[a]);
			a++;
		}
	...
	printf("VanitySearch v" RELEASE " phrutis random min<->max edition 2022\n");
	...
	VanitySearch *v = new VanitySearch(secp, minn, maxx, prefix, seed, nbit, searchMode, coinType, gpuEnable, stop, outputFile, sse,
