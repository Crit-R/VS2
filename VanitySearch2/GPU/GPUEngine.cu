...
/// 256
__global__ void comp_keys_256(uint32_t mode, prefix_t *prefix, uint32_t *lookup32, uint64_t *keys, uint32_t maxFound, uint32_t *found) {

	int xPtr = (blockIdx.x*blockDim.x) * 8;
	int yPtr = xPtr + 4 * blockDim.x;
	ComputeKeys256(mode, keys + xPtr, keys + yPtr, prefix, lookup32, maxFound, found);

}

__global__ void comp_keys_eth_256(uint32_t mode, prefix_t *prefix, uint32_t *lookup32, uint64_t *keys, uint32_t maxFound, uint32_t *found) {

	int xPtr = (blockIdx.x*blockDim.x) * 8;
	int yPtr = xPtr + 4 * blockDim.x;
	ComputeKeysETH256(mode, keys + xPtr, keys + yPtr, prefix, lookup32, maxFound, found);

}
__global__ void comp_keys_p2sh_256(uint32_t mode, prefix_t *prefix, uint32_t *lookup32, uint64_t *keys, uint32_t maxFound, uint32_t *found) {

	int xPtr = (blockIdx.x*blockDim.x) * 8;
	int yPtr = xPtr + 4 * blockDim.x;
	ComputeKeysP2SH256(mode, keys + xPtr, keys + yPtr, prefix, lookup32, maxFound, found);

}

__global__ void comp_keys_pattern_256(uint32_t mode, prefix_t *pattern, uint64_t *keys, uint32_t maxFound, uint32_t *found) {

	int xPtr = (blockIdx.x*blockDim.x) * 8;
	int yPtr = xPtr + 4 * blockDim.x;
	ComputeKeys256(mode, keys + xPtr, keys + yPtr, NULL, (uint32_t *)pattern, maxFound, found);

}

__global__ void comp_keys_p2sh_pattern_256(uint32_t mode, prefix_t *pattern, uint64_t *keys, uint32_t maxFound, uint32_t *found) {

	int xPtr = (blockIdx.x*blockDim.x) * 8;
	int yPtr = xPtr + 4 * blockDim.x;
	ComputeKeysP2SH256(mode, keys + xPtr, keys + yPtr, NULL, (uint32_t *)pattern, maxFound, found);

}

/// B

__global__ void comp_keys_b(uint32_t mode, prefix_t *prefix, uint32_t *lookup32, uint64_t *keys, uint32_t maxFound, uint32_t *found) {

	int xPtr = (blockIdx.x*blockDim.x) * 8;
	int yPtr = xPtr + 4 * blockDim.x;
	ComputeKeysB(mode, keys + xPtr, keys + yPtr, prefix, lookup32, maxFound, found);

}

__global__ void comp_keys_eth_b(uint32_t mode, prefix_t *prefix, uint32_t *lookup32, uint64_t *keys, uint32_t maxFound, uint32_t *found) {

	int xPtr = (blockIdx.x*blockDim.x) * 8;
	int yPtr = xPtr + 4 * blockDim.x;
	ComputeKeysETHB(mode, keys + xPtr, keys + yPtr, prefix, lookup32, maxFound, found);

}
__global__ void comp_keys_p2sh_b(uint32_t mode, prefix_t *prefix, uint32_t *lookup32, uint64_t *keys, uint32_t maxFound, uint32_t *found) {

	int xPtr = (blockIdx.x*blockDim.x) * 8;
	int yPtr = xPtr + 4 * blockDim.x;
	ComputeKeysP2SHB(mode, keys + xPtr, keys + yPtr, prefix, lookup32, maxFound, found);

}

__global__ void comp_keys_pattern_b(uint32_t mode, prefix_t *pattern, uint64_t *keys, uint32_t maxFound, uint32_t *found) {

	int xPtr = (blockIdx.x*blockDim.x) * 8;
	int yPtr = xPtr + 4 * blockDim.x;
	ComputeKeysB(mode, keys + xPtr, keys + yPtr, NULL, (uint32_t *)pattern, maxFound, found);

}

__global__ void comp_keys_p2sh_pattern_b(uint32_t mode, prefix_t *pattern, uint64_t *keys, uint32_t maxFound, uint32_t *found) {

	int xPtr = (blockIdx.x*blockDim.x) * 8;
	int yPtr = xPtr + 4 * blockDim.x;
	ComputeKeysP2SHB(mode, keys + xPtr, keys + yPtr, NULL, (uint32_t *)pattern, maxFound, found);
...
void GPUEngine::SetCoinType(int coinType) {
	this->coinType = coinType;
}

void GPUEngine::SetNBit(int nbit) {
	this->nbit = nbit;
}
...
bool GPUEngine::callKernel256() {
	...
	if (coinType == COIN_BTC) {
		...
		if (searchType == P2SH) {
			...
			if (hasPattern) {
				comp_keys_p2sh_pattern_256 << < nbThread / nbThreadPerGroup, nbThreadPerGroup >> >
					(searchMode, inputPrefix, inputKey, maxFound, outputPrefix);
			}
			else {
				comp_keys_p2sh_256 << < nbThread / nbThreadPerGroup, nbThreadPerGroup >> >
			...
			if (hasPattern) {
				...
				comp_keys_pattern_256 << < nbThread / nbThreadPerGroup, nbThreadPerGroup >> >
					(searchMode, inputPrefix, inputKey, maxFound, outputPrefix);
			}
			else {
				if (searchMode == SEARCH_COMPRESSED) {
					comp_keys_256 << < nbThread / nbThreadPerGroup, nbThreadPerGroup >> >
						(searchMode, inputPrefix, inputPrefixLookUp, inputKey, maxFound, outputPrefix);
				}
				else {
					comp_keys_256 << < nbThread / nbThreadPerGroup, nbThreadPerGroup >> >
		...
		comp_keys_eth_256 << < nbThread / nbThreadPerGroup, nbThreadPerGroup >> >
			(searchMode, inputPrefix, inputPrefixLookUp, inputKey, maxFound, outputPrefix);
	}
...
bool GPUEngine::callKernelB() {
	...
	if (coinType == COIN_BTC) {
		...
		if (searchType == P2SH) {
			...
			if (hasPattern) {
				comp_keys_p2sh_pattern_b << < nbThread / nbThreadPerGroup, nbThreadPerGroup >> >
					(searchMode, inputPrefix, inputKey, maxFound, outputPrefix);
			}
			else {
				comp_keys_p2sh_b << < nbThread / nbThreadPerGroup, nbThreadPerGroup >> >
					(searchMode, inputPrefix, inputPrefixLookUp, inputKey, maxFound, outputPrefix);
			}
			...
			if (hasPattern) {
				...
				comp_keys_pattern_b << < nbThread / nbThreadPerGroup, nbThreadPerGroup >> >
					(searchMode, inputPrefix, inputKey, maxFound, outputPrefix);
			}
			else {
				if (searchMode == SEARCH_COMPRESSED) {
					comp_keys_b << < nbThread / nbThreadPerGroup, nbThreadPerGroup >> >
						(searchMode, inputPrefix, inputPrefixLookUp, inputKey, maxFound, outputPrefix);
				}
				else {
					comp_keys_b << < nbThread / nbThreadPerGroup, nbThreadPerGroup >> >
		...
		comp_keys_eth_b << < nbThread / nbThreadPerGroup, nbThreadPerGroup >> >
			(searchMode, inputPrefix, inputPrefixLookUp, inputKey, maxFound, outputPrefix);
	}
...
bool GPUEngine::SetKeys(Point *p) {
	...
	return nbit == 256 ? callKernel256() : callKernelB();
