...
__device__ __noinline__  char *itohexa_helper(char *dest, unsigned x)
{
	if (x >= 16) {
...
__device__ __noinline__ char *itohexa(char *dest, unsigned x)
{
	*itohexa_helper(dest, x) = '\0';
	return dest;
}
__device__ __noinline__ void _GetAddressETH(uint32_t *hash, char *Add) {
